...
let supabaseImplicitClientSingleton: ReturnType<typeof createBrowserClient> | null = null;
...
export function getSupabaseImplicitClient() {
  ...
  if (!supabaseImplicitClientSingleton) {
    supabaseImplicitClientSingleton = createBrowserClient(supabaseUrl!, supabaseAnonKey!, {
      auth: {
        flowType: "implicit",
        persistSession: true,
        autoRefreshToken: true,
  ...
  return supabaseImplicitClientSingleton;
}
