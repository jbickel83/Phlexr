...
export async function fetchPostRow(postId) {
  const supabase = getSupabaseClient();
...
export async function fetchFollowingRows(followerUserId) {
  ...
  const errorMessage = String(detailedResult.error.message || "").toLowerCase();
  ...
  return supabase
    .from("follows")
    .select("id, follower_user_id, following_user_id, created_at")
    .eq("follower_user_id", followerUserId);
}

export async function fetchAllFollowRows() {
  const supabase = getSupabaseClient();
...
export async function castPostVote(postId, voteType) {
  const supabase = getSupabaseClient();

  const rpcResult = await supabase.rpc("cast_post_vote", {
    target_post_id: postId,
    next_vote_type: voteType,
  });

  const rpcErrorMessage = String(rpcResult.error?.message || "").toLowerCase();
  const shouldFallbackToDirectVote =
    rpcResult.error &&
    (rpcErrorMessage.includes("cast_post_vote") || rpcErrorMessage.includes("schema cache"));

  if (!shouldFallbackToDirectVote) {
    return rpcResult;
  }
  ...
  if (userError || !user) {
    return {
      data: null,
      error: userError || rpcResult.error,
    };
  }
  ...
  const voteResult = await supabase
    ...
    .upsert(
      ...
      { onConflict: "post_id,voter_user_id" }
    )
    .select("id")
    .single();
...
export function subscribeToFollows(callback) {
  if (!isSupabaseConfigured()) {
    return { unsubscribe() {} };
  }
